...
| **H2** | **Concentus is confirmed dead.** Zero case-insensitive hits across the whole `Plugin\` tree outside `manifest.json` and `thunderstore.toml`. No `Concentus.dll` in `Plugin\dlls\` (which holds exactly Coroner, EmployeeClasses, EnhancedMonsters, PathfindingLib). No `PackageReference`. Decoding is entirely NVorbis. **RULED (b7): drop it.** Remove from `thunderstore.toml` and `manifest.json`. Player-facing (mod managers stop installing it), so it ships **with a release and a CHANGELOG line**, not inside a code batch. Closes the open `ToDo.md` question. | ruled |
...
2. **Concentus (H2) → drop it.** Remove `qwbarch-Concentus = "2.3.0"` from `thunderstore.toml` and `"qwbarch-Concentus-2.3.0"` from `manifest.json`. **This is player-facing** — mod managers will stop installing it — so it belongs in a release with a CHANGELOG line, not slipped into a code batch.
