...
> - ~~**C3, the push/damage owner-authority fix.**~~ **APPLIED 2026-08-16 (b22), after Mathew confirmed the repro on a real host/client pair: clients are neither pushed nor damaged.** It was held back precisely because it was **PLAUSIBLE** and untestable from here; a second machine settled it. See the C3 row in *Batch C* below.
...
| 2 | **Push and push-damage on clients** | Applied server-side to a remote player's copy. Works on the host, evaporates for everyone else. Mathew tests as host, which is why this has never been seen. | **FIXED in b22 and VERIFIED in game 2026-08-16. CLOSED.** |
...
| **C3** | ~~Push and push-damage never reach clients.~~ **CONFIRMED IN GAME, FIXED IN b22, AND THE FIX VERIFIED IN GAME 2026-08-16 — clients are now pushed and take the damage. CLOSED.** Mathew ran the second-machine test the audit asked for and clients were neither pushed nor damaged. **The audit was right about the push and wrong to name `AttackPlayer` alongside it** — melee was already correct, because `OnCollideWithPlayer` reaches `AttackPlayer` only through `MeetsStandardPlayerCollisionConditions`, which filters to `localPlayerController`, so melee damage already runs on the victim's machine. **Only `PushPlayer` was broken, and both halves of it were.** Proven off the installed `Assembly-CSharp.dll` with `ilspycmd`, not inferred: `PlayerControllerB.DamagePlayer` opens `if (!base.IsOwner \|\| isPlayerDead \|\| !AllowPlayerDeath()) return;`, and `externalForceAutoFade` is consumed only inside `PlayerControllerB.Update` behind `base.IsOwner && isPlayerControlled && (!base.IsServer \|\| isHostPlayerObject)`. `PushPlayer` runs server-only, so both writes hit the server's replica of a remote player and were never read. **Fix: the server still decides the push and computes the force vector, then hands it to the victim's machine over a new `ApplyPushClientRpc(int playerId, Vector3 pushForce)`**, which filters on `localPlayerController.playerClientId` and applies damage plus knockback there. Broadcast-and-filter rather than `ClientRpcParams`, deliberately: it costs a few bytes to the other clients and avoids mapping player index to NGO client id. The host path is unchanged in effect, because a ClientRpc also executes on the host. **This added an RPC, so the netcode patcher must have run** — a hand-copied DLL will no-op the push for everyone. | **FIXED (b22)** |
