...
| `Docs\Plans\Pathfinding_Audit.md` | **LIVE.** Deep dive on navigation only (board b10, 2026-08-16) — how much of PathfindingLib the mod actually uses, and what is broken or wasteful in the nav path. **Read-only; batches N1–N6 are proposed and NOT applied.** Its headline is that **PathfindingLib is used for the least valuable thing it does**: the multi-destination, off-thread, true-path-length API is never called, while the three searches that need it run synchronous `CalculatePath` up to 120 times a call; and three of the four smart-link flags are switched off by a hardcoded constant, so the Gargoyle can never use a fire exit, the main entrance, or the mineshaft elevator. Every claim was verified by decompiling PathfindingLib 2.4.1 and the game's `Assembly-CSharp.dll` — bodies are quoted. **Also carries a *Corrections* section: `Optimization_Audit.md` is STALE and lists G2/D1/D2 as unapplied when they shipped in `c0c0189`/`a1dac4b`.** Start at *The one-paragraph answer*, then the headline table. |
...
| `.claude\skills\` | **Five procedure skills** — `voice-line`, `game-update`, `ship-release`, `playtest`, `changelog-writer` — plus the two model-routing lanes `deep-work` and `light-touch`. The procedure skills carry checklists that would otherwise be re-derived; the routing lanes only move the model dial. `CLAUDE.md` → *Procedure Skills* says when each fires. |
