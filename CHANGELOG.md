...
- Measured with six Gargoyles alive at once, across a full round: four frames went over a 60fps budget and one went over 30fps. That was a debug build, which carries timing instrumentation the version you install does not.
...
- Fixed several ways he could freeze on the spot, one of which could hold him for half a minute at a time. Most of them shared a cause, which was two parts of the AI steering him at once. His roaming search was left running when he switched to stalking you, and when he later lost track of you, the route to wherever he had been heading was left running against the fresh search. The two kept cancelling each other, so he recalculated constantly and never followed anything. Separately, when he could not find cover he was told to walk to where he was already standing, and recalculated that same answer forever. He now hands the steering over cleanly on every change of behaviour. Not yet verified in a live round.
- Fixed him freezing whenever you stood at almost exactly `General > Idle Distance` from him. He is supposed to stop moving at that range, but the switch was a single hard line, so one step of yours flipped him between standing still and stalking, over and over. In a recorded round he held the same position for thirteen straight AI updates while a player wandered around him from ten metres out to twenty. He now settles at the Idle Distance and does not get moving again until you have opened up about a fifth further. Not yet verified in a live round.
...
- `Pathfinding > Agent Radius` (default `0.5`). How much room his steering thinks he needs. His model is built at `1.25`, and this now overrides it, which is what stopped him jamming on the spot at corners and doorways and refusing to squeeze past railings. Raise it toward `1.25` if you would rather he kept more distance from walls and from the other Gargoyles.
