...
public class AudioManager : NetworkBehaviour
{
                ...
                if (AudioClipEnableConfig.TryGetValue(clipName, out ConfigEntry<bool> configEntry) && configEntry.Value)
                {
                    ...
                    if (audioData != null)
                    {
                        Task sendTask = Task.Run(() =>
                        {
                            ...
                            try
                            {
                                FastBufferWriter writer = new(totalBufferSize, allocator);
    ...
    private async void SendAudioClipToClient(ulong clientId, byte[] audioData, string clipName, string category, FastBufferWriter writer)
    {
        ...
        if (writer.Capacity < sizeof(int))
        {
            Plugin.Logger.LogError("Not enough space in buffer for fragment length!");
            return;
        }

        writer.WriteValueSafe(audioData.Length); // Write fragment length
        writer.WriteBytesSafe(audioData); // Fragment data
